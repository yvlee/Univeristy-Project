class ArrayBasedList:
    ...
    def isFull(self):
        ...
        '''
        if (self.items >= len(self.myArray)):
            return True
        return False
    ...
    def insert(self,index,item):
        ...
        if not noSpace and validIndex:
            self.myArray += [None]                 #increase list size
            self.items += 1
            for i in range (self.items-1,index,-1):
                self.myArray[i] =  self.myArray[i-1]    #moving items
            self.myArray[index] = item
            return self.myArray
        else:
            raise IndexError ("Your list is full")
    ...
    def delete(self,index):
        ...
        validIndex = index >= 0 and index < self.items
        if validIndex:
            for i in range (index, self.items-1):
